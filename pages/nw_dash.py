...
nw_price_df = pd.DataFrame(nw_sell_records)

# Ensure date column is properly typed.
nw_price_df['Date'] = pd.to_datetime(nw_price_df['Date'])

# Parse numerical price value and convert to float
nw_price_df['Price'] = nw_price_df['Price'].astype(float)

# Ensure 'USD' column is calculated.
# 1 chinese yuan equals 0.1433 USD https://www.currency.me.uk/convert/cny/usd
nw_price_df['USD'] = nw_price_df['Price'] * 0.1434

# Calculate global mean/min data.
# lost_ark_global_info = pd.read_csv(constants.LA_GLOBAL_INFO)
global_means = nw_price_df.groupby(pd.Grouper(key='Date', axis=0, freq='D'))['USD'].mean()
global_means = global_means.rename('mean')
global_mins = nw_price_df.groupby(pd.Grouper(key='Date', axis=0, freq='D'))['USD'].min()
...
server_means = nw_price_df.groupby(['Server', 'Date'])['USD'].mean().reset_index().rename(columns={'USD': 'mean'})
server_mins = nw_price_df.groupby(['Server', 'Date'])['USD'].min().reset_index().rename(columns={'USD': 'min'})
...
region_means = nw_price_df.groupby(['Region', 'Date'])['USD'].mean().reset_index().rename(columns={'USD': 'mean'})
region_mins = nw_price_df.groupby(['Region', 'Date'])['USD'].min().reset_index().rename(columns={'USD':'min'})
...
names_df = nw_price_df.groupby(['Date', 'Seller'])['USD'].count().reset_index().sort_values(by=['USD', 'Seller', 'Date'],
                                                                                         ascending=False)
...
nw_helper_df = nw_price_df
new_world_seller = nw_helper_df.groupby(['Seller', 'Date'])['USD'].mean().reset_index()
...
layout = html.Div(children=[
    ...
    html.Div(children='''
        Historical gold prices: New World
    '''),
    ...
    html.Div([
        dcc.Graph(id='new_world_server_and_region_prices'),
        'Select a server to view gold prices: ',
        dcc.Dropdown(
            id='new_world_select_server',
            options=constant.NEW_WORLD_SERVER_DROPDOWN_OPTIONS,
            multi=False,
            value='Aaru',
            style={'width': '40%'}
        ),
        ...
        dcc.Dropdown(
            id='new_world_select_region',
            options=[
                {'label': 'EU Central', 'value': 'EU_Central'},
                {'label': 'AP Southeast', 'value': 'AP_Southeast'},
                {'label': 'US East', 'value': 'US_East'},
                {'label': 'US West', 'value': 'US_West'},
                {'label': 'South America East', 'value': 'SA_East'}
            ],
            multi=False,
            value='US_West',
            style={'width': '40%'}
...
@dash.callback(
    Output(component_id='new_world_server_and_region_prices', component_property='figure'),
    Input(component_id='new_world_select_server', component_property='value'),
    Input(component_id='new_world_select_region', component_property='value')
)
...
@dash.callback(
    Output(component_id='new_world_individual_seller_info', component_property='figure'),
    Input(component_id='new_world_user_input', component_property='value')
)
def update_seller_info(seller_name):
    temp = current_names.copy()

    filtered_df6 = new_world_seller[new_world_seller['Seller'] == seller_name]
